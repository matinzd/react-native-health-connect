...
- The whole `android-expo/` Expo module and its `expo-module.config.json` were removed. The module
  existed only to register the delegate from a `ReactActivityLifecycleListener`, so nothing was
  left for it to do. Expo apps now autolink `android/` like any other React Native package;
  `app.plugin.js` is unaffected. Verified by a local `expo prebuild` + `./gradlew projects`.
